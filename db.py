...
# ===================== ІНІЦІАЛІЗАЦІЯ БД =====================
async def ensure_users_table_and_columns():
    """ОДНА функція, яка створює таблицю + ВСІ колонки (включаючи balance)"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            CREATE TABLE IF NOT EXISTS users (
                user_id INTEGER PRIMARY KEY,
...
async def ensure_balance_column():
    """Додає колонку balance якщо її немає"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("PRAGMA table_info(users)") as cursor:
            columns = {row[1] for row in await cursor.fetchall()}
        if "balance" not in columns:
            ...
            await db.commit()
            print("✅ Додано колонку balance в users")


async def create_pending_payments_table():
    """Створює таблицю для очікуваних платежів Monobank"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
        ...
        """)
        await db.commit()
        print("✅ Таблиця pending_payments готова")


async def create_used_monobank_txs_table():
    """🆕 Створює таблицю для відслідковування використаних TX"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
        ...
        await db.execute(
            "CREATE INDEX IF NOT EXISTS idx_user_id ON used_monobank_txs(user_id)"
        )
        
        await db.commit()
        print("✅ Таблиця used_monobank_txs готова")
...
async def init_db():
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            await ensure_users_table_and_columns()
            await ensure_balance_column()
            await create_pending_payments_table()
            await create_used_monobank_txs_table()  # 🆕 ДОДАЄМО!
...
async def add_pending_payment(user_id: int, amount_kop: int, comment: str, mono_account: str = "0"):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
...
async def get_pending_payments() -> list[dict]:
    """Повертає всі активні pending платежі"""
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("""
...
async def is_tx_used(tx_id: str) -> bool:
    """🛡️ Проверяем, была ли эта транзакция уже использована"""
    async with aiosqlite.connect(DB_PATH) as db:
        result = await db.execute(
            "SELECT 1 FROM used_monobank_txs WHERE tx_id = ?",
            (tx_id,)
        )
...
async def get_used_txs_for_user(user_id: int) -> list:
    """Получаем список уже использованных TX для пользователя"""
    async with aiosqlite.connect(DB_PATH) as db:
        result = await db.execute(
            "SELECT tx_id FROM used_monobank_txs WHERE user_id = ? ORDER BY used_at DESC",
            (user_id,)
        )
