...
# ==================== ЄДИНИЙ DB_PATH ====================
DATA_DIR = os.environ.get("DATA_DIR") or Path.home() / "TgBotData"
DB_PATH = Path(DATA_DIR) / "users.db"
...
# ===================== ІНІЦІАЛІЗАЦІЯ БД =====================
async def ensure_users_table_and_columns():
    """Єдина функція — створює таблицю + ВСІ колонки"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            CREATE TABLE IF NOT EXISTS users (
                user_id INTEGER PRIMARY KEY,
        ...
        if "promo" not in columns:
            await db.execute("ALTER TABLE users ADD COLUMN promo INTEGER DEFAULT 0")
            print("✅ Додано колонку: promo (промокоди)")
...
async def get_promo(user_id: int) -> int:
    """Повертає кількість промокодів у користувача"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT promo FROM users WHERE user_id = ?", (user_id,)) as cursor:
            row = await cursor.fetchone()
            return row[0] if row else 0


async def spend_promo(user_id: int) -> bool:
    """Списує 1 промокод. Повертає True, якщо успішно"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT promo FROM users WHERE user_id = ?", (user_id,)) as cursor:
            row = await cursor.fetchone()
            if not row or row[0] < 1:
                return False
        await db.execute("UPDATE users SET promo = promo - 1 WHERE user_id = ?", (user_id,))
        await db.commit()
        return True


async def add_promo(user_id: int, amount: int = 1):
    """Додає промокоди користувачу"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET promo = promo + ? WHERE user_id = ?",
            (amount, user_id)
        )
        await db.commit()
        print(f"✅ Додано {amount} промо користувачу {user_id}")
