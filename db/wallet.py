...
# ) -> bool:
#     """
#     Повертає True якщо TX успішно зарезервована під цього user_id.
#     Повертає False якщо TX вже існує (для цього або іншого користувача).
    
#     Це запобігає тому, щоб два користувачі з однаковою сумою не зарахували
#     одну й ту ж транзакцію з Monobank.
#     """
#     async with aiosqlite.connect(DB_PATH) as db:
#         cursor = await db.execute(
#             """
...
#     if inserted:
#         logging.info(f"🔐 TX успішно зарезервована: tx_id='{tx_id}' для user_id={user_id}")
#         return True
#     else:
#         # TX вже існує в таблиці — перевіримо, чия вона
#         async with aiosqlite.connect(DB_PATH) as db:
#             cur = await db.execute(
...
#                     logging.warning(
#                         f"⚠️ TX вже зарезервована під цього користувача: "
#                         f"tx_id='{tx_id}' user_id={user_id}"
#                     )
#                 else:
#                     logging.warning(
#                         f"🚫 TX ЗАНЯТА ІНШИМ КОРИСТУВАЧЕМ! "
#                         f"tx_id='{tx_id}' вже під user_id={existing_user_id}, "
#                         f"але спробував user_id={user_id}"
#                     )
#             else:
#                 # Це не повинно трапитися, але на всяк випадок
#                 logging.warning(f"⚠️ TX вже використана: tx_id='{tx_id}'")
