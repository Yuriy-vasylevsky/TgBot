...
# Імпорт з db (тепер усе через ensure_users_table_and_columns)
from db import (
...
@router.message(F.text == "/bonus")
async def bonus_entry(message: Message):
    await ensure_users_table_and_columns()          # ← тепер звідси
...
async def spin_bonus(cb: CallbackQuery):
    ...
    # Перевірка кількості зіграних ігор
    try:
        user_data = await get_user_data(user_id)
        games_played = user_data.get("games_played", 0) if user_data else 0
    except:
    ...
    # Перевірка дати останнього бонусу
    last_date = await get_last_bonus_date(user_id)
    today = datetime.date.today().isoformat()
    ...
    try:
        ...
        prize = _choose_bonus()
        await update_last_bonus_date(user_id)               # зберігаємо дату
...
# ===============================
#  Робота з БД (правильні функції)
# ===============================
async def get_last_bonus_date(user_id: int) -> str | None:
    """Отримує дату останнього щоденного бонусу"""
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT last_daily_bonus_date FROM users WHERE user_id = ?",
            (user_id,),
