...
async def handle_basketball_dice(message: Message):
    ...
    if hits >= BASKETBALL_HITS_TO_WIN:
        # Перевірка кулдауну
        ...
        try:
            await message.bot.send_message(
                chat_id=user_id,
                text=(
                    f"🎉 <b>Вітаємо з перемогою в Баскетболі!</b>\n\n"
                    f"Твій приз:\n"
                    f"<code>{promo}</code>\n\n"
        ...
        if sent_to_private:
            win_text = (
                f"🎉 <b>ПЕРЕМОЖЕЦЬ!</b> 🏆\n\n"
                f"{user.mention_html()} влучив <b>{BASKETBALL_HITS_TO_WIN} рази</b>!\n\n"
                f"🏀 🏀 🏀"
            )
        else:
            win_text = (
    ...
    progress_msg = await message.answer(
        f"🏀 {user.mention_html()} — <b>{hits}/{BASKETBALL_HITS_TO_WIN} влучних кидків</b>",
        parse_mode="HTML"
    )
