...
#     # async def add_to_terminal(self, terminal_id: int, amount: int) -> Dict:
#     #     """Поповнити термінал (за аналогією з getBalanceTerminal/collectTerminal)"""
#     #     token = await self._auth()
#     #     session = await self._get_session()

#     #     url = f"{BASE_URL}/addToTerminal"
#     #     headers = {"Token": token}
#     #     payload = {
#     #         "id": terminal_id,
#     #         "amount": amount * 100  # копійки, за аналогією з addCodes
#     #     }

#     #     async with session.post(url, json=payload, headers=headers) as resp:
#     #         text = await resp.text()
#     #         print(f"[ADDTOTERMINAL] Status: {resp.status} | Body: {text[:400]}")

#     #         if resp.status != 200:
#     #             raise Exception(f"addToTerminal error {resp.status}: {text[:400]}")

#     #         return self._parse_json(text)

#     # async def add_to_check_by_code(self, code: str, amount: int) -> Dict:
#     #     """Поповнює чек через terminal_id (addToCode не існує — 'unknown path')"""
#     #     terminal_id = await self.get_terminal_id(code)
#     #     return await self.add_to_terminal(terminal_id, amount)
    

#     async def get_currencies(self) -> Dict:
#         """Список валют системи — треба, щоб знайти currencyId для UAH"""
#         token = await self._auth()
#         session = await self._get_session()
