...
from handlers.menu import main_menu
from handlers.config import ADMIN_ID
from games.fortune import fortune_keyboard, FORTUNE_COST
...
@router.callback_query(F.data == "profile:fortune")
async def cb_open_fortune(callback: types.CallbackQuery):
    """Відкриває Колесо фортуни прямо з кабінету (інлайн-кнопка)."""
    user_id = callback.from_user.id

    await callback.answer()

    promo = await get_promo(user_id)
