...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        🔹 Чотири виміри АЗАРТУ!
        🔹 Знайди їх у SLOTS
        🔹 Приєднуйтесь до світу ігор і відчуйте азарт!""",
        reply_markup=main_menu()
    )

async def send_ref(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(config.REF)

async def send_group_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f'Приєднуйтесь до нашої групи: {config.GROUP_LINK}')

async def send_money(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f'Для виводу напишіть нашому касиру: {config.CONTACT_PHONE}')

async def send_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f'Касир: {config.CONTACT_PHONE}')

async def send_card_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(config.CARD_NUMBER)

async def send_halp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(config.HALP)

async def send_demo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(config.DEMO)

async def send_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(config.LINK1)
    await update.message.reply_text(config.LINK2)
    await update.message.reply_text(config.LINK3)

async def send_mb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_photo(photo='./1.jpg')
    await update.message.reply_text(config.AK1)

async def send_seif(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_photo(photo='./2.jpg')
    await update.message.reply_text(config.AK2)

async def send_cash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_photo(photo='./3.jpg')
    await update.message.reply_text(config.AK3)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if code_pattern.match(text):
        code = text.replace('-', '')
        await update.message.reply_text(f'Чемпіон https://spinplanet.net/?login_code={code}')
        await update.message.reply_text(f'Суперматік https://code.greenhost.pw/?c={code}')
        return

    mapping = {
        '🎲 Група': send_group_link,
        '💎 Касир': send_contact,
        '💳 Номер карти': send_card_number,
        '❓ Як грати': send_halp,
        '🔹 Акції': lambda u, c: u.message.reply_text('Оберіть акцію:', reply_markup=actions_menu()),
        '🎴 Посилання на ігри': send_link,
        '💫 КОД в посилання': send_ref,
        '💲 Вивід': send_money,
        '💥 Демо гра': send_demo,
        '🎮 Морський бій': send_mb,
        '🎲 Сейф': send_seif,
        '🃏 Cash Back': send_cash,
        '🔙 Повернутись до головного меню': lambda u, c: u.message.reply_text(
            f'Якщо питання – пишіть касиру {config.CONTACT_PHONE}', reply_markup=main_menu())
    }
    ...
    if text in mapping:
        ...
        await update.message.reply_text(
            "Будь ласка, використовуйте меню або введіть код у форматі: 00-00-00-00-00-00-00"
        )
