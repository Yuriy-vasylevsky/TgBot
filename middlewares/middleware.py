...
class BanMiddleware(BaseMiddleware):
    """
    ⚠️ ЗМІНЕНО: бан більше НЕ блокує користувача глобально.
    Раніше цей middleware обривав усі повідомлення/колбеки забаненого юзера
    ще до того, як апдейт доходив до конкретного хендлера — через це бан
    впливав на весь бот, а не тільки на Matic.

    Тепер middleware нічого не блокує і завжди пропускає апдейт далі.
    Перевірка бану винесена туди, де вона й повинна бути — у Matic-хендлери
    в handlers/admin_checks.py (функція _matic_blocked), які викликають
    is_banned() з handlers/admin_bans.py.

    Клас залишено (не видалено), щоб не ламати реєстрацію в main.py,
    якщо вона десь лишилась, а не тому що він щось робить.
    """
